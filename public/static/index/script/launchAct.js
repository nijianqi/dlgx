...
function setPickerStyle(e, t) {
    var i = $(".dpicker-body").eq(t).scrollTop(), a = parseInt(i / 38);
    if (a === e)return e;
...
$(function () {
    ...
    var t = 0, i = 0, a = 0, r = 0, n = [1, 3, 5, 7, 8, 10, 12];
    $(".dpicker-body").scrollTop("9"), $(".dpicker-body").eq(0).scroll(function () {
        ...
        var e = $(".dpicker-item.active"), t = e.eq(0).text() + " 年 " + e.eq(1).text() + " 月 " + e.eq(2).text() + " 日 ";
        $(".date-picker").css({bottom: "-17.5rem"}), $(".show-dpicker input").eq(+r).val(t)
    }), $(".dpicker-cancel").click(function () {
        $(".date-picker").css({bottom: "-17.5rem"})
    }), $(".show-dpicker").click(function () {
        r = $(this).attr("data-id"), $(".date-picker").css({bottom: 0})
    }), setPickerStyle(0), $("#club-logo-set").prev("input").change(function () {
        var e = $(this).val(), t = e.substring(e.lastIndexOf(".") + 1);
