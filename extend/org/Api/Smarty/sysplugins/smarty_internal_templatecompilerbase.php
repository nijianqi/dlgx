...
abstract class Smarty_Internal_TemplateCompilerBase {
    ...
    /**
     * js loaded by default plugin handler
     *
     * @var array
     */
     ...
     * Compile Tag
     *
     * This is a call backup from the lexer/parser
                ...
                foreach ($this->smarty->plugin_search_order as $plugin_type) {
                    if ($plugin_type == Smarty::PLUGIN_BLOCK && $this->smarty->loadPlugin('smarty_compiler_' . $tag) && (!isset($this->smarty->security_policy) || $this->smarty->security_policy->isTrustedTag($tag, $this))) {
                        ...
                        if (is_callable($plugin)) {
                            // convert arguments format for old compiler js
                            $new_args = array();
                            foreach ($args as $key => $mixed) {
                            	if (is_array($mixed)) {
                                	$new_args = array_merge($new_args, $mixed);
    ...
    /**
     * Check for js and return function name
     *
    ...
    /**
     * Check for js by default plugin handler
     *
    ...
    public function processNocacheCode($content, $is_code)
    {
        ...
        if ($is_code && !empty($content)) {
            ...
            ($this->nocache || $this->tag_nocache || $this->forceNocache == 2)) {
                ...
                // make sure we include modifer js for nocache code
