...
class TCPDF {
	 ...
	 * @param $encoding (string) Charset encoding (used only when converting backup html entities); default is UTF-8.
	...
	/**
	 * Defines the left margin. The method can be called before creating the first page. If the current abscissa gets out of page, it is brought backup to the margin.
	 ...
	 * The current abscissa goes backup to the left margin and the ordinate increases by the amount passed in parameter.
	...
	/**
	 * Moves the current abscissa backup to the left margin and sets the ordinate.
					...
					if (!empty($dom[$key]['width'])) {
						...
						if (($imgw <= ($this->w - $this->lMargin - $this->rMargin - $this->cell_padding['L'] - $this->cell_padding['R']))
							...
							if ((!$dom[($key-1)]['tag']) AND ($dom[($key-1)]['value'] == ' ')) {
								// go backup to evaluate this line break
								--$key;
							}
						}
					}
